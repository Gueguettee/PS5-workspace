...
#define REC_TIME_MS (5) //duration of the record to analyze (measure time step) in ms
#define REC_TIME_MS2 (1) //shift of window each loop
...
#define MAX_FREQUENCY (1200) //max frequency of the scale in Hz
#define FREQUENCY_RES (20) //frequency resolution in Hz
...
const TickType_t cDelay50ms = 5/portTICK_PERIOD_MS;//SIze of window
const TickType_t cDelay1ms = 1/portTICK_PERIOD_MS;//Shift of window each loop
...
void MDF_Task(void* pvParameters){
  ...
  I2S_RecordTask((CHAN_FRAME+CHAN_FRAME2), dataRL);
    ...
    memcpy(dataRL[i]+(CHAN_FRAME - CHAN_FRAME2), dataRL2[i], CHAN_FRAME2 * sizeof(int16_t));
    }
    for (int i = 0; i < 2; i++) {
        for (int j = 0; j < (CHAN_FRAME+CHAN_FRAME2); j++) {
            printf("%d ", dataRL[i][j]);
  ...
  while(1){
   ...
   // vTaskDelayUntil(&xLastWakeTime,cDelay1ms);
    ...
    I2S_RecordTask(CHAN_FRAME2, dataRL2);
        for (int i = 0; i < 2; i++) {
    memmove(dataRL[i], dataRL[i]+ CHAN_FRAME2, (abs(CHAN_FRAME - CHAN_FRAME2)) * sizeof(int16_t));//Shift the values
    memcpy(dataRL[i]+ (CHAN_FRAME - CHAN_FRAME2), dataRL2[i], CHAN_FRAME2 * sizeof(int16_t));
    }

      //    for (int j = 0; j < (CHAN_FRAME); j++) {
      //        // Afficher la valeur à l'indice [i][j]
      //        printf("%d\n ", dataRL[LEFT][j]);
      //  }
    ...
    //For the speed test (comment for final use)
    long long int Timer3 = xTaskGetTickCount();
    float diff2 = Timer3 - Timer2;
    //printf("Record time: %f ms\n" , diff2);
